...
/**
 * Extract profession from entity ID (works for both items and crafts)
 * @param {string} entityId - Entity ID in format TYPE:PROFESSION:IDENTIFIER
 * @returns {string|null} - Profession name or null if invalid format
 */
export function extractProfessionFromId(entityId) {
...
/**
 * Get profession color from entity ID
 * @param {string} entityId - Entity ID in format TYPE:PROFESSION:IDENTIFIER
 * @returns {string} - Hex color or default color if profession not found
 */
export function getProfessionColorFromId(entityId) {
  const profession = extractProfessionFromId(entityId);
  if (!profession) return '#75715e'; // Default gray
  
  const professions = store.getState().professions;
  // Professions is an array, find by profession name
  const professionData = professions.find(p => p.id === `profession:${profession}`);
  return professionData?.color || '#75715e'; // Default gray if profession not found
}
